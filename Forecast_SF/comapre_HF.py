...
for coun in range(len(df_input_sub.columns)):
    # If the last h_train observations of training data (2020) are not flat, run Shape finder, else pass
...
for coun in range(len(df_input_sub.columns)):
    # If the last h_train observations of training data (2021) are not flat, run Shape finder, else pass
    if not (df_input_sub.iloc[-h_train:,coun]==0).all():
        shape = Shape()
        # Set last h_train observations of training data as shape
        shape.set_shape(df_input_sub.iloc[-h_train:,coun]) 
...
### Using 2020 to make predictions in 2021 ###
...
df_input_sub=df_input.iloc[:-24]
for coun in range(len(df_input_sub.columns)):
    # If the last h_train observations of training data (2020) are not flat, esle return flat predictions
    if not (df_input_sub.iloc[-h_train:,coun]==0).all():
        ...
        # Get mean sequence for each cluster
...
df_input_sub=df_input.iloc[:-12]
for coun in range(len(df_input_sub.columns)):
    # If the last h_train observations of training data (2021) are not flat, esle return flat predictions
    if not (df_input_sub.iloc[-h_train:,coun]==0).all():
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(preds, label='Shape Finder', marker='o',color='purple')
        plt.plot(df_preds_test_2.iloc[:12,coun].reset_index(drop=True), label='ViEWS', marker='o',color='darkgreen')
        plt.plot(df_input.iloc[-12:,coun].reset_index(drop=True),label='Actuals',marker='o',color="black")
        plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.08), ncol=5,fontsize=12)
        plt.title(df_input_sub.columns[coun])
        plt.grid(True)
        #plt.savefig(os.path.join(out_paths["analysis"],f"compare_preds_{coun}.jpeg"),dpi=400,bbox_inches="tight")
...
#df_sf_2.to_csv('sf2.csv') # 2022
        
# Actuals       
df_obs_1 = df_input.iloc[-24:-12,:]        
df_obs_1.to_csv('obs1.csv') # 2021
df_obs_2 = df_input.iloc[-12:,:]        
#df_obs_2.to_csv('obs2.csv') # 2022

# Views predictions
df_v_1 = df_preds_test_1.iloc[:12,:]
df_v_1.to_csv('views1.csv') # 2021
df_v_2 = df_preds_test_2.iloc[:12,:]
#df_v_2.to_csv('views2.csv') # 2022
...
for coun in range(len(df_input_sub.columns)):
    # If the last h_train observations of training data (2020) are not flat
    if not (df_input_sub.iloc[-h_train:,coun]==0).all():
...
for coun in range(len(df_input_sub.columns)):
    # If the last h_train observations of training data (2021) are not flat
    if not (df_input_sub.iloc[-h_train:,coun]==0).all():
