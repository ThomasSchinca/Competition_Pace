# -*- coding: utf-8 -*-
"""
Created on Mon Sep 25 13:38:21 2023
...
year=['2021','2020','2019','2018']
yea=2
i=pd.DataFrame(d_diff).mean().sort_values(ascending=False).index[2]
real = tot_df_obs.iloc[y*12:(y+1)*12,i]
sf=tot_df_nn.iloc[y*12:(y+1)*12,i]
b1=tot_df_ar.iloc[y*12:(y+1)*12,i]
b2=tot_df_ar2.iloc[y*12:(y+1)*12,i]
plt.figure(figsize=(15, 10))
plt.plot(sf, label='ShapeFinder',color='r')
plt.plot(real,color='b',linewidth=5,label='Obs value')
plt.legend()
plt.grid(True)
plt.xticks([])
plt.yticks([])
...
tot = pd.read_csv('boot.csv',index_col=(0))
tot_3 = tot[tot.year==3]
tot_3['week']=tot_3['week']+int(bench1_tot.index[0])
tot_3['country']=tot_3['country'].map(pd.Series(bench1_tot.columns))
tot_3['country']=tot_3['country'].astype(int)
tot_3=tot_3[['week','country','boot','value']]
tot_3.columns = ['month_id', 'country_id', 'draw','outcome']
tot_3.to_parquet('pred_2018_15bootstrap.parquet',index=False)

tot_3 = tot[tot.year==2]
tot_3['week']=tot_3['week']+int(bench1_tot.index[0])+12
tot_3['country']=tot_3['country'].map(pd.Series(bench1_tot.columns))
tot_3['country']=tot_3['country'].astype(int)
tot_3=tot_3[['week','country','boot','value']]
tot_3.columns = ['month_id', 'country_id', 'draw','outcome']
tot_3.to_parquet('pred_2019_15bootstrap.parquet',index=False)

tot_3 = tot[tot.year==1]
tot_3['week']=tot_3['week']+int(bench1_tot.index[0])+24
tot_3['country']=tot_3['country'].map(pd.Series(bench1_tot.columns))
tot_3['country']=tot_3['country'].astype(int)
tot_3=tot_3[['week','country','boot','value']]
tot_3.columns = ['month_id', 'country_id', 'draw','outcome']
tot_3.to_parquet('pred_2020_15bootstrap.parquet',index=False)

tot_3 = tot[tot.year==0]
tot_3['week']=tot_3['week']+int(bench1_tot.index[0])+36
tot_3['country']=tot_3['country'].map(pd.Series(bench1_tot.columns))
tot_3['country']=tot_3['country'].astype(int)
tot_3=tot_3[['week','country','boot','value']]
tot_3.columns = ['month_id', 'country_id', 'draw','outcome']
tot_3.to_parquet('pred_2021_15bootstrap.parquet',index=False)
